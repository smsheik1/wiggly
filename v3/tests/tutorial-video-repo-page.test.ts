...
assert.equal(
  ...
  "https://github.com/smsheik1/wiggly-tutorial-video/releases/download/v0.4.0/wiggly-tutorial-video-format-kit-0.4.0.zip",
);
...
assert.ok(profile.repositoryHref.startsWith("http") || existsSync(`public${profile.repositoryHref}`));
assert.ok(readFileSync("public/format-repositories/tutorial-video-v1/PROOF-REPORT.md", "utf8").includes("Failure addressed"));

const localZip = `public/format-repositories/tutorial-video-v1/downloads/wiggly-tutorial-video-format-kit-0.4.0.zip`;
if (existsSync(localZip)) {
  const archiveBytes = readFileSync(localZip);
