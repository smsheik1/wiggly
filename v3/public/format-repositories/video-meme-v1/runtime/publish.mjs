...
const USAGE = "Usage: node runtime/publish.mjs <inputs/distribution.json> [goldens/bear-secret.mp4] [--dry-run]";
...
async function main() {
  ...
  const mediaPath = positionalArgs[1] 
    ...
    : resolve(dirname(inputPath), "..", distConfig.media || "goldens/bear-secret.mp4");
  ...
  if (platforms.youtube?.enabled) {
    ...
    if (!yt.categoryId) {
      validationErrors.push("YouTube requires a numeric categoryId (e.g. \"23\" for Comedy or \"24\" for Entertainment).");
    }
