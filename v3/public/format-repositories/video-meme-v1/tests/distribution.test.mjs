...
test("publish.mjs generates unconfigured_environment receipt without BUFFER_API_KEY", async () => {
  const receiptPath = path.join(root, "goldens/bear-secret.mp4.distribution.json");
  ...
  try {
    ...
    assert.equal(receipt.mediaSha256, "09d04e65a5bf5f335b1fd9d20284b756c57101859b217e246798d0026c57eaa0");
  } finally {
    if (existsSync(receiptPath)) {
      await unlink(receiptPath);
    }
