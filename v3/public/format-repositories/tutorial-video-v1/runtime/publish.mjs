...
const USAGE = "Usage: node runtime/publish.mjs <inputs/distribution.json> [examples/animal-conversations-first-run/final.mp4] [--dry-run]";
...
async function main() {
  ...
  const mediaPath = positionalArgs[1] 
    ...
    : resolve(dirname(inputPath), "..", distConfig.media || "examples/animal-conversations-first-run/final.mp4");
  ...
  if (platforms.youtube?.enabled) {
    ...
    if (!yt.title || yt.title.trim().length === 0) {
      validationErrors.push("YouTube requires a non-empty title.");
    } else if (yt.title.length > 100) {
      validationErrors.push(`YouTube title exceeds 100 characters (${yt.title.length}).`);
    }
    if (!yt.categoryId) {
      validationErrors.push("YouTube requires a numeric categoryId (e.g. \"27\" for Education or \"28\" for Science & Tech).");
    }
    if (yt.type === "short" && videoStats && videoStats.duration > 60) {
  ...
  if (platforms.instagram?.enabled) {
    ...
    if (!ig.caption || ig.caption.trim().length === 0) {
      validationErrors.push("Instagram requires a caption.");
    }
    if (ig.type === "reel" && videoStats && (videoStats.width > videoStats.height)) {
