...
test("pipeline.json includes distribute stage with approvalRequired", async () => {
  ...
  const stage = pipeline.stages.find(s => s.id === "distribute" || s.id === "Distribute");
...
test("publish.mjs validates inputs and executes cleanly in --dry-run mode", () => {
  const stdout = execFileSync("node", [
    ...
    "examples/animal-conversations-first-run/final.mp4"
...
test("publish.mjs generates unconfigured_environment receipt without BUFFER_API_KEY", async () => {
  const receiptPath = path.join(root, "examples/animal-conversations-first-run/final.mp4.distribution.json");
  ...
  try {
    ...
    assert.equal(receipt.mediaSha256, "1c9293b3b8cfcc25d713f31625974ef339ffdbfa6ce9cb4b3c7e4f807c2734b6");
  } finally {
    if (existsSync(receiptPath)) {
      await unlink(receiptPath);
    }
