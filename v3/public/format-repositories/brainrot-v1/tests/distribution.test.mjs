...
test('publish.mjs generates unconfigured_environment receipt without BUFFER_API_KEY', async () => {
  const receiptPath = path.join(root, 'goldens/davids-cookies.mp4.distribution.json');
  ...
  try {
    ...
    assert.equal(receipt.mediaSha256, '82061eb1f0a610bc2eaeff1a637173e71d27c4062c271fda9dd8f398c47e5f13');
  } finally {
    if (existsSync(receiptPath)) {
      await unlink(receiptPath);
    }
