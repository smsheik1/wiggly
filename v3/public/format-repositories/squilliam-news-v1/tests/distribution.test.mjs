...
test("publish.mjs validates inputs and executes cleanly in --dry-run mode", () => {
  const stdout = execFileSync("node", [
    ...
    "examples/we-the-artists/evidence/final.mp4"
...
test("publish.mjs generates unconfigured_environment receipt without BUFFER_API_KEY", async () => {
  const receiptPath = path.join(root, "examples/we-the-artists/evidence/final.mp4.distribution.json");
  ...
  try {
    ...
    assert.equal(receipt.mediaSha256, "4225ac87c1d6db293579502c2ffe5b660a6351aa8b74e16b471060f7e80657db");
  } finally {
    if (existsSync(receiptPath)) {
      await unlink(receiptPath);
    }
