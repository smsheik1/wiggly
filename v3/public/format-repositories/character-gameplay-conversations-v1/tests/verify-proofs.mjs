...
const run=(tool,args)=>execFileSync(tool,['-v','error','-protocol_whitelist','file,pipe',...args],{timeout:30000,maxBuffer:4*1024*1024});
const results=[];
for(const [index,file]of files.entries()){
 ...
 const video=probe.streams.find(s=>s.codec_type==='video');assert.equal(video.width,480);assert.equal(video.height,640);assert.equal(video.r_frame_rate,'25/1');assert.ok(Math.abs(Number(probe.format.duration)-frequencies.length*2)<0.08);assert.ok(probe.streams.some(s=>s.codec_type==='audio'));
 ...
 for(const [i,hz]of frequencies.entries()){
  ...
  assert.equal(rgb.length,480*640*3);assert.ok(rgb[0]>240&&rgb[1]>240&&rgb[2]>240,'Header corner should be white');
  let white=0;for(let k=450*480*3;k<525*480*3;k+=3)if(rgb[k]>220&&rgb[k+1]>220&&rgb[k+2]>220)white++;assert.ok(white>100,'Expected visible white caption pixels');
  captionFrames.push(createHash('sha256').update(rgb.subarray(415*480*3,525*480*3)).digest('hex'));
 }
 assert.ok(new Set(captionFrames).size===frequencies.length,'Caption/speaker regions should differ across turns');results.push({file,durationSeconds:Number(probe.format.duration),width:480,height:640,fps:25,measuredAudio:measured,captionRegionHashes:captionFrames});
}
