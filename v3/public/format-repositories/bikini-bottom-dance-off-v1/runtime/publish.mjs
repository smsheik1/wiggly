...
const USAGE = "Usage: node runtime/publish.mjs <inputs/distribution.json> [examples/wiggle-proof/evidence/final.mp4] [--dry-run]";
...
async function main() {
  ...
  const mediaPath = positionalArgs[1] 
    ...
    : resolve(dirname(inputPath), "..", distConfig.media || "examples/wiggle-proof/evidence/final.mp4");
  ...
  if (platforms.youtube?.enabled) {
    ...
    if (!yt.categoryId) {
      validationErrors.push("YouTube requires a numeric categoryId (e.g. \"20\" for Gaming or \"23\" for Comedy).");
    }
