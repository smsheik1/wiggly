...
test("publish.mjs validates inputs and executes cleanly in --dry-run mode", () => {
  const stdout = execFileSync("node", [
    ...
    "examples/wiggle-proof/evidence/final.mp4"
...
test("publish.mjs generates unconfigured_environment receipt without BUFFER_API_KEY", async () => {
  const receiptPath = path.join(root, "examples/wiggle-proof/evidence/final.mp4.distribution.json");
  ...
  try {
    ...
    assert.equal(receipt.mediaSha256, "9410930282548bc052dc9123d5b84ea7df9b1a4d9809f491b3441a2cc7ca5620");
  } finally {
    if (existsSync(receiptPath)) {
      await unlink(receiptPath);
    }
