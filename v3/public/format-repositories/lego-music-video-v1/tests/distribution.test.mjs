...
test("publish.mjs generates unconfigured_environment receipt without BUFFER_API_KEY", async () => {
  const receiptPath = path.join(root, "goldens/cookies/final.mp4.distribution.json");
  ...
  try {
    ...
    assert.equal(receipt.mediaSha256, "c306a5dc3cb6f9acca72a4a08bd638aefd6e8cd7c5a360d80811d65c1497d90c");
  } finally {
    if (existsSync(receiptPath)) {
      await unlink(receiptPath);
    }
