...
const USAGE = "Usage: node runtime/publish.mjs <inputs/distribution.json> [goldens/cookies/final.mp4] [--dry-run]";
...
async function main() {
  ...
  const mediaPath = positionalArgs[1] 
    ...
    : resolve(dirname(inputPath), "..", distConfig.media || "goldens/cookies/final.mp4");
  ...
  if (platforms.youtube?.enabled) {
    ...
    if (!yt.categoryId) {
      validationErrors.push("YouTube requires a numeric categoryId (e.g. \"10\" for Music or \"24\" for Entertainment).");
    }
